...
echo %~n0...
title %~n0...
make -j5 SUBTARGET=mess OSD=winui PTR64=1 BUILDDIR=build_mess
cd %~dp0
if exist arcade64.exe (echo %date% %time% %~n0 OK! >> 程序编译情况.txt) else (echo %date% %time% %~n0 NO! >> 程序编译情况.txt)
ren arcade64.exe arcade64_mess.exe
xcopy /y /f arcade64_mess.exe %~dp0\ArcadeProgramFiles\ >> 程序编译情况.txt
::pause
